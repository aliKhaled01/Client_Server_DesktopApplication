...
def purchase(data_obj, conn, addr):
    ...
    c.execute("SELECT quantity FROM books  WHERE name = '{}'".format(item_name))
    check_quantity = c.fetchone()[0]

    c.execute("SELECT quantity FROM books WHERE name = '{}'".format(item_name))
    needed_quantity = c.fetchone()[0]

    if check_quantity >= needed_quantity:
        user_lock.acquire()
        ...
        print("Unsucessful buy operation")
